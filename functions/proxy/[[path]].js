...
export async function onRequest(context) {
    ...
    // 获取远程内容及其类型
    // 自动根据 Content-Type 决定以文本还是二进制模式读取
    async function fetchContentWithType(targetUrl) {
        ...
        try {
            ...
            // 根据 Content-Type 自动决定读取方式
            if (isTextContent(contentType)) {
                // 文本模式：读取为文本（用于 M3U8、JSON 等）
                ...
                // 二进制模式：使用 arrayBuffer 保留原始二进制数据（图片、视频等）
                const arrayBuffer = await response.arrayBuffer();
                logDebug(`请求成功(二进制): ${targetUrl}, Content-Type: ${contentType}, 大小: ${arrayBuffer.byteLength}`);
                return { arrayBuffer, contentType, responseHeaders: response.headers, isBinary: true };
            }
    ...
    // 判断是否是文本内容（只有文本内容才需要 M3U8 处理和文本缓存）
    function isTextContent(contentType) {
        ...
        return ct.includes('text/') || ct.includes('json') || ct.includes('xml') ||
            ...
            ct.includes('audio/mpegurl') || ct.includes('javascript') || ct.includes('css');
    }
    ...
    async function processMasterPlaylist(url, content, recursionDepth, env) {
        ...
        const variantResult = await fetchContentWithType(bestVariantUrl);
        const variantContentType = variantResult.contentType;

        // 如果子列表返回二进制内容，说明不是 M3U8，直接返回二进制数据
        if (variantResult.isBinary) {
            logDebug(`获取到的子列表 ${bestVariantUrl} 返回二进制内容 (类型: ${variantContentType})，直接代理返回。`);
            const binHeaders = new Headers();
            binHeaders.set('Content-Type', variantContentType || 'application/octet-stream');
            binHeaders.set('Access-Control-Allow-Origin', '*');
            binHeaders.set('Cache-Control', `public, max-age=${CACHE_TTL}`);
            return new Response(variantResult.arrayBuffer, {
                status: 200,
                headers: binHeaders
            });
        }

        const variantContent = variantResult.content;
    ...
    try {
        ...
        // 如果是二进制内容（图片、视频、音频等），直接返回二进制数据
        if (fetchResult.isBinary) {
            logDebug(`非文本内容 (类型: ${contentType})，直接返回二进制数据: ${targetUrl}`);
            const finalHeaders = new Headers();
            // 只保留必要的安全头信息，避免 Content-Encoding/Content-Length 不匹配
            // 因为 arrayBuffer() 已经解压了内容，长度和编码可能与原始响应不同
            const safeOriginalHeaders = [
                'content-type', 'cache-control', 'expires', 'last-modified', 'etag'
            ];
            responseHeaders.forEach((value, key) => {
                if (safeOriginalHeaders.includes(key.toLowerCase())) {
