// 豆瓣图片代理配置
const DOUBAN_IMAGE_PROXY_CONFIG = {
    defaultType: 'ali',
    localStorageKey: 'doubanImageProxyType',
    typeLabels: {
        server: '服务器代理 (支持缓存)',
        ali: '阿里 CDN (推荐)',
        tencent: '腾讯 CDN',
    ...
    aliHost: 'img.doubanio.cmliussss.com',
    tencentHost: 'img.doubanio.cmliussss.net',
};

// 获取当前图片代理类型
function getDoubanImageProxyType() {
    return localStorage.getItem(DOUBAN_IMAGE_PROXY_CONFIG.localStorageKey) || DOUBAN_IMAGE_PROXY_CONFIG.defaultType;
}

// 处理豆瓣图片 URL，根据代理类型返回不同的 URL
function processDoubanImageUrl(originalUrl) {
    if (!originalUrl || !originalUrl.includes('doubanio.com')) return originalUrl;

    const proxyType = getDoubanImageProxyType();
    switch (proxyType) {
        case 'server':
            // 通过服务器代理加载（支持缓存）
        ...
        case 'ali':
            // 阿里 CDN 镜像
            return originalUrl.replace(/img\d+\.doubanio\.com/g, DOUBAN_IMAGE_PROXY_CONFIG.aliHost);
        case 'tencent':
            // 腾讯 CDN 镜像
            return originalUrl.replace(/img\d+\.doubanio\.com/g, DOUBAN_IMAGE_PROXY_CONFIG.tencentHost);
        case 'img3':
            // 替换为 img3
...
function initDouban() {
    ...
    if (doubanToggle) {
        ...
        // 初始化图片代理设置可见性
        updateImageProxySettingVisibility(isEnabled);
        ...
        doubanToggle.addEventListener('change', function (e) {
            ...
            // 更新图片代理设置可见性
            updateImageProxySettingVisibility(isChecked);
...
// 根据豆瓣开关状态显示/隐藏图片代理设置
function updateImageProxySettingVisibility(isEnabled) {
    const proxySetting = document.getElementById('doubanImageProxySetting');
    if (proxySetting) {
        if (isEnabled) {
            proxySetting.classList.remove('hidden');
        } else {
            proxySetting.classList.add('hidden');
        }
    }
}

// 初始化图片代理选择器
function initDoubanImageProxySelect() {
    const select = document.getElementById('doubanImageProxySelect');
    if (!select) return;

    // 从 localStorage 读取当前值
    const currentType = getDoubanImageProxyType();
    select.value = currentType;

    // 监听变化
    select.addEventListener('change', function () {
        localStorage.setItem(DOUBAN_IMAGE_PROXY_CONFIG.localStorageKey, this.value);
        showToast(`图片代理已切换为：${DOUBAN_IMAGE_PROXY_CONFIG.typeLabels[this.value] || this.value}`, 'success');

        // 如果豆瓣功能已启用，重新加载内容以应用新的代理方式
        if (localStorage.getItem('doubanEnabled') === 'true') {
            const doubanArea = document.getElementById('douban-results');
            if (doubanArea && doubanArea.children.length > 0) {
                renderRecommend(doubanCurrentTag, doubanPageSize, doubanPageStart);
            }
        }
    });
...
async function fillAndSearchWithDouban(title) {
    ...
    // 确保豆瓣资源API被选中
    if (typeof selectedAPIs !== 'undefined' && !selectedAPIs.includes('1')) {
        // 在设置中勾选豆瓣资源API复选框
        const doubanCheckbox = document.querySelector('input[id="api_1"]');
        if (doubanCheckbox) {
