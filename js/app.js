...
// 迁移检查：如果存储的API key在新配置中不存在，则重置为全选
if (selectedAPIs.length === 0 || selectedAPIs.some(key => !key.startsWith('custom_') && !API_SITES.hasOwnProperty(key))) {
    selectedAPIs = [...Array(20).keys()].map(String);
...
async function search() {
    ...
    try {
        ...
        const safeResults = allResults.map(item => {
            ...
            // 使用代理加载图片，解决 Cloudflare 等平台跨域加载失败的问题
            const proxiedPicUrl = hasCover ? PROXY_URL + encodeURIComponent(item.vod_pic) : '';
                ...
                <div class="card-hover bg-[#111] rounded-lg overflow-hidden cursor-pointer transition-all hover:scale-[1.02] h-full shadow-sm hover:shadow-md" 
                    ...
                    <div class="flex h-full">
                        ...
                        <div class="relative flex-shrink-0 search-card-img-container">
                            <img src="${proxiedPicUrl}" alt="${safeName}" 
                                 class="h-full w-full object-cover transition-transform hover:scale-110" 
                                 onerror="this.onerror=null; this.src='${item.vod_pic}'; this.onerror=function(){this.src=\'https://via.placeholder.com/300x450?text=无封面\';}; this.classList.add('object-contain');" 
                                 loading="lazy">
...
// 从data属性获取参数并调用playVideo函数
function playVideoFromData(button) {
    const url = button.getAttribute('data-url');
    const vodName = button.getAttribute('data-vodname');
